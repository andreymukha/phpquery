...
class phpQueryObjectPlugin_WebBrowser
{
    ...
    public static function WebBrowser($self, $callback = null, $location = null)
    {
        ...
        if (!$location) {
            ...
            $self->bind('click', [$location, $callback], ['phpQueryPlugin_WebBrowser', 'hadleClick']);
            $self->bind('submit', [$location, $callback], ['phpQueryPlugin_WebBrowser', 'handleSubmit']);
        }
    }
...
class phpQueryPlugin_WebBrowser
{
     ...
     * @throws Exception
     */
    public static function browserGet(
        $url,
        $callback,
        $param1 = null,
        $param2 = null,
...
//			$urlRefresh = urldecode($urlRefresh);
            ...
            if ($xhr->getLastResponse()->isSuccessful()) {
                ...
                return call_user_func_array(
                    ['phpQueryPlugin_WebBrowser', 'browserReceive'],
                    [$xhr]
                );
            }
        } else {
...
//			));
    ...
    public static function hadleClick($e, $callback = null)
    {
        ...
        if ($node->is('a[href]')) {
            ...
            if ($node->is(':submit') && $node->parents('form')->size()) {
