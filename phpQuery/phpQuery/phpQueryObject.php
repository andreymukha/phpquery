...
//			var_dump(phpQuery::$documents);
            throw new Exception(
                "Document with ID '{$id}' isn't loaded. Use phpQuery::newDocument(\$html) or phpQuery::newDocumentFile(\$file) first."
            );
        }
        $this->documentID = $id;
...
//				$this->elements = $stack;
            ...
            case 'header':
                $this->elements = $this->map(
                    function ($node) {
                        $isHeader = isset($node->tagName) && in_array(
                                $node->tagName,
                                [
                                    "h1",
                                    "h2",
...
//				)->elements;
            ...
            case 'nth-child':
                ...
                if ($param == 'even' || $param == 'odd') {
                    ...
                    if (mb_strlen($param) > 1 && preg_match('/^(\d*)n([-+]?)(\d*)/', $param) === 1) // an+b
                    {
                        $mapped = $this->map(
                            function ($node, $param) {
...
//			foreach($node->getElementsByTagName('*') as $newNode) {
    ...
    /**
     * @param null $attr
     * @return array text content of each matching element, like
    ...
    /**
     * @param null $attr
     * @return array same attribute of each matching element, like
